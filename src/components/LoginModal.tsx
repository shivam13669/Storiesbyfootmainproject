...
export const LoginModal = ({ isOpen, onClose }: LoginModalProps) => {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Save user data to Firestore with role and approval status
      ...
      if (error.code === "auth/email-already-in-use") {
        setGeneralError("Email already in use. Please login or use a different email.");
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      ...
      // This supports both signup and login scenarios
      if (user) {
        try {
          await createUserDocument(user.uid, {
            email: user.email || "",
          ...
          console.log("Firestore save skipped (user may already exist):", firestoreError);
        }
      }
                  ...
                  <div className="p-7 space-y-6">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      <button
                        ...
                        disabled={isLoading}
                        className="border-2 border-gray-200 text-gray-700 font-semibold py-2.5 px-4 rounded-lg hover:border-orange-200 hover:bg-orange-50/30 transition-all duration-200 flex items-center justify-center gap-2 group disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {isLoading ? (
                        ...
                        <span className="text-sm">{isLoading ? "Signing in..." : "Google"}</span>
                      </button>
                    </div>
